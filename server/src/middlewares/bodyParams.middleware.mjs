...
export default function handlingBodyParams({ mapOfKeyValues={}, optional={} }={}) {
  return (req, res, next) => {
    // Checking the types
    if (
    ...
    Object.entries(optional).forEach(([key, default_value]) => {
      if (
        typeof request_data[key] === 'string' &&
        request_data[key].trim().length === 0
      ) {
        request_data[key] = default_value;
        
        req.body[key] = default_value;
      }
    });
    ...
    request_keys.forEach(key => {
      ...
      if (casted_type && !valid_types.includes(casted_type)) {
        return res.status(400).json({
